...
export class TodoService {
  ...
  async updateTodo(input: UpdateTodoInput): Promise<TodoDTO> {
    ...
    const preConditionCheck = (cache: TodosCache): void => {
      ...
      // Pass the new dueDate if it's being updated, so validation checks against the updated value
      if (input.status !== undefined && input.status !== existingEntity.status) {
        const newDueDate = input.dueDate !== undefined ? input.dueDate : existingEntity.dueDate;
        this.validateStatusTransition(existingEntity, input.status, newDueDate);
      }
   ...
   * @param newDueDate - Optional new due date (if being updated in same request)
   * @throws Error if transition is invalid
   */
  private validateStatusTransition(entity: TodoEntity, newStatus: StoredStatus, newDueDate?: string | null): void {
    const currentDate = getCurrentDateUTC();
    
    // Use new dueDate if provided, otherwise use entity's current dueDate
    const effectiveDueDate = newDueDate !== undefined ? newDueDate : entity.dueDate;
    
    // Calculate status using the effective dueDate (the one that will be used after update)
    const entityWithEffectiveDueDate: TodoEntity = {
      ...entity,
      dueDate: effectiveDueDate,
    };
    const calculatedStatus = calculateStatus(entityWithEffectiveDueDate, currentDate);

    // Cannot transition from calculated "due" to "initial" if due date is in past
    if (calculatedStatus === "due" && newStatus === "initial") {
      if (effectiveDueDate !== null && effectiveDueDate < currentDate) {
