...
function createGeminiClient(config, options) {
  ...
  return {
     ...
     */
    async generateContent(newContents, saveConversation = true) {
...
function validateSchema(s, path, isRootSchema = false) {
  const schema = s;
...
var WAP_SCHEMA_TYPE_TO_GEMINI_TYPE_MAP, RENDER_TOOL_FOR_GEMINI;
...
async function executeRenderTool(params, geminiClient) {
  ...
  const text = candidate.content?.parts?.map((part) => part.text && !part.thought ? part.text : "").join("") || "";
  if (!text) {
...
async function executeRender(functionCall, geminiClient) {
...
function extractText(candidate) {
  const parts = candidate?.content?.parts || [];
  const textParts = parts.filter((part) => part.text !== void 0).map((part) => part.text).join("");
...
async function executeFunctionCallsInOrder(functionCalls, apiClient, renderGeminiClient, callbacks, isAborted, setPendingUserAction, handleUserAction) {
...
function orchestrate(userInput, config, callbacks) {
...
${toolsBundle.examples.map((example, index) => `## Example ${index + 1}:
...
async function initOrchestrator(apiKey, apiBaseUrl, callbacks) {
