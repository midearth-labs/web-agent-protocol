...
// src/api-client/manifest.type.ts
function validateWAPManifest(m) {
  const manifest = m;
  ...
  const toolNameMap = new Set(manifest.tools.map((t) => t.name));
  manifest.userJourneys.forEach((journey, index) => {
    validateUserJourney(journey, index, toolNameMap);
  });
}
function validateUserJourney(j, index, toolNameMap) {
  const journey = j;
...
function validateTool(t, index) {
  ...
  if (tool.parameters === void 0 || tool.parameters === null) {
  ...
  const hasUpdateOrDelete = tool.tags.includes("update" /* UPDATE */) || tool.tags.includes("delete" /* DELETE */);
  if (tool.response !== void 0 && tool.response !== null) {
...
function validateSchema(s, path2, isRootSchema = false) {
  const schema = s;
  if (!schema || typeof schema !== "object") {
    throw new WAPManifestValidationError(`${path2} must be an object`);
  }
  if (schema.type === void 0 || schema.type === null) {
    throw new WAPManifestValidationError(`${path2}.type must be defined`);
  }
  if (typeof schema.type !== "string") {
    throw new WAPManifestValidationError(`${path2}.type must be a string`);
  }
  if (!Object.values(WAPSchemaType).includes(schema.type)) {
    throw new WAPManifestValidationError(
      `${path2}.type must be one of: ${Object.values(WAPSchemaType).join(", ")}`
    );
  }
  if (schema.format !== void 0 && schema.format !== null) {
    if (typeof schema.format !== "string") {
      throw new WAPManifestValidationError(`${path2}.format must be a string`);
    }
    const validFormats = VALID_FORMATS[schema.type];
    if (validFormats.length > 0 && !validFormats.includes(schema.format)) {
      throw new WAPManifestValidationError(
        `${path2}.format "${schema.format}" is not valid for type "${schema.type}". Valid formats: ${validFormats.join(", ")}`
      );
    }
  }
  if (schema.type === "ARRAY" /* ARRAY */) {
    if (schema.items === void 0 || schema.items === null) {
      throw new WAPManifestValidationError(`${path2}.items must be defined when type is ARRAY`);
    }
    validateSchema(schema.items, `${path2}.items`);
  }
  if (schema.type === "OBJECT" /* OBJECT */) {
    const properties = schema.properties;
    if (properties === void 0 || properties === null) {
      throw new WAPManifestValidationError(`${path2}.properties must be defined when type is OBJECT`);
    }
    if (typeof properties !== "object" || Array.isArray(properties)) {
      throw new WAPManifestValidationError(`${path2}.properties must be an object`);
    }
    Object.entries(properties).forEach(([propName, propSchema]) => {
      validateSchema(propSchema, `${path2}.properties.${propName}`);
    });
    if (schema.required !== void 0 && schema.required !== null) {
      if (!Array.isArray(schema.required)) {
        throw new WAPManifestValidationError(`${path2}.required must be an array`);
      }
      schema.required.forEach((requiredProp, i) => {
        if (typeof requiredProp !== "string") {
          throw new WAPManifestValidationError(`${path2}.required[${i}] must be a string`);
        }
        if (!(requiredProp in properties)) {
          throw new WAPManifestValidationError(
            `${path2}.required[${i}] references property "${requiredProp}" that does not exist in properties`
          );
        }
      });
    }
  }
  if (schema.anyOf !== void 0 && schema.anyOf !== null) {
    if (!Array.isArray(schema.anyOf)) {
      throw new WAPManifestValidationError(`${path2}.anyOf must be an array`);
    }
    schema.anyOf.forEach((anyOfSchema, i) => {
      validateSchema(anyOfSchema, `${path2}.anyOf[${i}]`);
    });
  }
  if (schema.enum !== void 0 && schema.enum !== null) {
    if (!Array.isArray(schema.enum)) {
      throw new WAPManifestValidationError(`${path2}.enum must be an array`);
    }
    schema.enum.forEach((enumValue, index) => {
      if (typeof enumValue !== "string") {
        throw new WAPManifestValidationError(`${path2}.enum[${index}] must be a string`);
      }
    });
  }
  if (schema.examples !== void 0 && schema.examples !== null) {
    if (!Array.isArray(schema.examples)) {
      throw new WAPManifestValidationError(`${path2}.examples must be an array`);
    }
  } else if (isRootSchema) {
    throw new WAPManifestValidationError(`${path2}.examples must be defined for root schema`);
  }
  if (schema.minItems !== void 0 && schema.minItems !== null) {
    const minItemsValue = schema.minItems;
    if (typeof minItemsValue === "string" && !/^\d+$/.test(minItemsValue)) {
      throw new WAPManifestValidationError(`${path2}.minItems ("${minItemsValue}") must be a base10 non-negative integer string`);
    }
  }
  if (schema.maxItems !== void 0 && schema.maxItems !== null) {
    const maxItemsValue = schema.maxItems;
    if (typeof maxItemsValue === "string" && !/^\d+$/.test(maxItemsValue)) {
      throw new WAPManifestValidationError(`${path2}.maxItems ("${maxItemsValue}") must be a base10 non-negative integer string`);
    }
  }
  if (schema.minLength !== void 0 && schema.minLength !== null) {
    const minLengthValue = schema.minLength;
    if (typeof minLengthValue === "string" && !/^\d+$/.test(minLengthValue)) {
      throw new WAPManifestValidationError(`${path2}.minLength ("${minLengthValue}") must be a base10 non-negative integer string`);
    }
  }
  if (schema.maxLength !== void 0 && schema.maxLength !== null) {
    const maxLengthValue = schema.maxLength;
    if (typeof maxLengthValue === "string" && !/^\d+$/.test(maxLengthValue)) {
      throw new WAPManifestValidationError(`${path2}.maxLength ("${maxLengthValue}") must be a base10 non-negative integer string`);
    }
  }
  if (schema.minimum !== void 0 && schema.minimum !== null) {
    const minimumValue = schema.minimum;
    if (typeof minimumValue !== "number") {
      throw new WAPManifestValidationError(`${path2}.minimum must be a number`);
    }
  }
  if (schema.maximum !== void 0 && schema.maximum !== null) {
    const maximumValue = schema.maximum;
    if (typeof maximumValue !== "number") {
      throw new WAPManifestValidationError(`${path2}.maximum must be a number`);
    }
  }
  if (schema.nullable !== void 0 && schema.nullable !== null) {
    if (typeof schema.nullable !== "boolean") {
      throw new WAPManifestValidationError(`${path2}.nullable must be a boolean`);
    }
  }
}
var WAPSchemaType, WAPManifestValidationError, VALID_FORMATS;
var init_manifest_type = __esm({
  "src/api-client/manifest.type.ts"() {
    "use strict";
    WAPSchemaType = /* @__PURE__ */ ((WAPSchemaType2) => {
      WAPSchemaType2["TYPE_UNSPECIFIED"] = "TYPE_UNSPECIFIED";
      WAPSchemaType2["STRING"] = "STRING";
      WAPSchemaType2["NUMBER"] = "NUMBER";
      WAPSchemaType2["INTEGER"] = "INTEGER";
      WAPSchemaType2["BOOLEAN"] = "BOOLEAN";
      WAPSchemaType2["ARRAY"] = "ARRAY";
      WAPSchemaType2["OBJECT"] = "OBJECT";
      WAPSchemaType2["NULL"] = "NULL";
      return WAPSchemaType2;
    })(WAPSchemaType || {});
    WAPManifestValidationError = class extends Error {
      constructor(message) {
        super(message);
    ...
    VALID_FORMATS = {
      ...
      ["STRING" /* STRING */]: ["uuid", "date", "date-time", "email", "byte", "enum", "uri", "uri-reference"],
...
// src/api-client/manifest.transform.ts
function manifestToGemini(manifest) {
  const functionDeclarations = [
...
function manifestToClaudeTools(manifest) {
  const tools = [
...
function buildFewShots(manifest) {
  return manifest.userJourneys.map((j) => userJourneyToFewShot(j));
}
function userJourneyToFewShot(journey) {
...
function wapSchemaToGeminiSchema(wapSchema) {
...
function wapSchemaToClaudeSchema(wapSchema) {
  const { examples, anyOf, type, items, properties, ...rest } = wapSchema;
  const schema = {
    ...rest
  };
...
var WAP_SCHEMA_TYPE_TO_GEMINI_TYPE_MAP, RENDER_TOOL_FOR_GEMINI, WAP_SCHEMA_TYPE_TO_JSON_SCHEMA_TYPE, RENDER_TOOL_FOR_CLAUDE;
var init_manifest_transform = __esm({
  "src/api-client/manifest.transform.ts"() {
    "use strict";
    init_manifest_type();
    init_web();
    WAP_SCHEMA_TYPE_TO_GEMINI_TYPE_MAP = {
      ...
      ["NULL" /* NULL */]: Type.NULL,
      ["TYPE_UNSPECIFIED" /* TYPE_UNSPECIFIED */]: Type.TYPE_UNSPECIFIED
    };
    RENDER_TOOL_FOR_GEMINI = {
      name: "render",
      description: "Generate dynamic UI render function for displaying substep results. Returns JavaScript code for function render(data, onAction). You must pass both the data structures (type definitions) and the actual data to render.",
      parameters: {
        type: Type.OBJECT,
        properties: {
          dataStructure: {
            type: Type.STRING,
            description: "TypeScript type definitions as a string. Includes inline descriptive comments for each property, even inner properties."
          },
          data: {
            type: Type.STRING,
            description: "The JSON string of the actual data to render. Structure must align with the typescript structure defined in dataStructure. This is the data that will be passed to the generated render function."
          },
    ...
    WAP_SCHEMA_TYPE_TO_JSON_SCHEMA_TYPE = {
      ...
      ["NULL" /* NULL */]: "null",
      ["TYPE_UNSPECIFIED" /* TYPE_UNSPECIFIED */]: "object"
      // Default to object
    };
    RENDER_TOOL_FOR_CLAUDE = {
      name: "render",
...
// src/orchestrator/gemini-client.ts
var gemini_client_exports = {};
__export(gemini_client_exports, {
...
function createGeminiProvider(config) {
  ...
  function fromGeminiCandidate(candidate) {
    ...
    const thinkingParts = parts.filter(
      (part) => part.thought === true && part.text !== void 0
    );
    const thinking = thinkingParts.length > 0 ? thinkingParts.map((part) => part.text).join("\n") : null;
    const functionCalls = [];
    ...
    const textParts = parts.filter((part) => part.text !== void 0 && !part.thought).map((part) => part.text);
  ...
  return {
    async generateContent(messages, saveConversation = true) {
    ...
    createFunctionResponseMessages(calls, responses) {
      const messages = [];
...
var init_batches2 = __esm({
  "node_modules/@anthropic-ai/sdk/resources/messages/batches.mjs"() {
    ...
    init_error2();
    init_path();
    Batches3 = class extends APIResource {
      /**
       * Send a batch of Message creation requests.
       *
       * The Message Batches API can be used to process multiple Messages API requests at
...
// src/orchestrator/claude-client.ts
...
function createClaudeProvider(config) {
  ...
  return {
    async generateContent(messages, saveConversation = true) {
      const claudeMessages = messages.map(toClaudeMessage);
...
// src/orchestrator/llm-provider.ts
async function createLLMProvider(config) {
  if (config.provider === "gemini") {
    const { createGeminiProvider: createGeminiProvider2 } = await Promise.resolve().then(() => (init_gemini_client(), gemini_client_exports));
    return createGeminiProvider2({
      apiKey: config.apiKey,
      options: config.options
    });
  } else if (config.provider === "claude") {
    const { createClaudeProvider: createClaudeProvider2 } = await Promise.resolve().then(() => (init_claude_client(), claude_client_exports));
  ...
  "src/api-client/api-client.ts"() {
    ...
    ApiClient2 = class {
      ...
      async request(options) {
        const { method, path: path2, params, query, body } = options;
        let urlPath = path2;
        if (params) {
        ...
        if (body !== void 0 && (method === "POST" || method === "PATCH")) {
        ...
        if (!response.ok) {
          const errorMessage = (typeof data === "object" && data !== null && "error" in data && typeof data.error === "object" && data.error !== null && "message" in data.error && typeof data.error.message === "string" ? data.error.message : null) || `HTTP ${response.status}: ${response.statusText}`;
          throw new Error(errorMessage);
        }
        return data;
      }
...
// src/orchestrator/render-executor.ts
...
function extractJavaScriptFunction(response) {
...
function extractTypeScriptFunction(response) {
  let code = response.trim();
...
function validateRenderFunction(code) {
...
async function executeRenderTool(params, llmProvider) {
...
var init_render_executor = __esm({
  "src/orchestrator/render-executor.ts"() {
    "use strict";
  }
});

// src/orchestrator/tool-executor.ts
async function executeSiteAPI(functionCall, apiClient) {
  const { name, args: argsOrUndefined } = functionCall;
  if (name === void 0) {
    throw new Error("Function call name is undefined. This should not happen.");
  }
  const args = argsOrUndefined;
  try {
    let result;
    switch (name) {
      case "createTodo": {
      ...
      case "listTodos": {
        ...
        if (args["dueDateBefore"]) query["dueDateBefore"] = args["dueDateBefore"];
        if (args["dueDateAfter"]) query["dueDateAfter"] = args["dueDateAfter"];
      ...
      case "updateTodo": {
        result = await apiClient.updateTodo({
          ...
          body: {
            ...
            ...args["dueDate"] !== void 0 && { dueDate: args["dueDate"] },
...
async function executeRender(functionCall, llmProvider) {
...
var init_tool_executor = __esm({
  "src/orchestrator/tool-executor.ts"() {
    "use strict";
    init_api_client();
    init_render_executor();
  }
});

// src/orchestrator/system-instruction.ts
var SYSTEM_INSTRUCTION;
var init_system_instruction = __esm({
  "src/orchestrator/system-instruction.ts"() {
    "use strict";
    SYSTEM_INSTRUCTION = `You are a WAP (Web Agent Protocol) orchestrator running in a browser. Your job is to 
...
function isRenderFunctionCall(call) {
  if (call.name !== "render") {
    return false;
  }
  const args = call.args;
  return typeof args === "object" && args !== null && "dataStructure" in args && "data" in args && "mainGoal" in args && "subGoal" in args && "stepType" in args && "actions" in args;
}
function requiresUserAction(renderCall) {
...
function executeRenderFunction(renderCode, renderData, handleUserAction) {
...
async function processRenderWithUserAction(renderCall, renderCode, callbacks, isAborted, setPendingUserAction, handleUserAction) {
  if (isAborted()) {
  ...
  if (requiresUserAction(renderCall)) {
    ...
    setPendingUserAction({ promise: actionPromise, resolve, reject });
    try {
...
async function executeFunctionCallsInOrder(functionCalls, apiClient, renderProvider, callbacks, isAborted, setPendingUserAction, handleUserAction) {
  ...
  if (taskCompleted) {
    ...
    completionError.responses = orderedResponses;
    throw completionError;
  }
  return orderedResponses;
}
async function orchestrate(userInput, config, callbacks) {
...
Assistant: ${example.assistant}`).join("\n\n")}` : "";
  ...
  const apiClient = new ApiClient2({
    baseUrl: apiBaseUrl
  });
...
// src/api-client/manifest.loader.ts
async function fetchManifest(url = "/wap.json") {
...
// src/orchestrator/browser.ts
var browser_exports = {};
__export(browser_exports, {
  initBrowserOrchestrator: () => initBrowserOrchestrator
});
async function initBrowserOrchestrator() {
  try {
    ...
    const callbacks = {
      ...
      onRenderRetry: async (error, _renderCall) => {
...
async function initOrchestrator(apiKey, apiBaseUrl, provider, callbacks) {
...
var init_index = __esm({
  "src/orchestrator/index.ts"() {
    ...
    if (typeof window !== "undefined") {
      Promise.resolve().then(() => init_browser());
    }
  }
});
init_index();
